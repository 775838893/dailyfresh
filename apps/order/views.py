        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                for i in range(3):
                    ...
                    # todo: 更新商品的库存和销量，悲观锁方式
                    # sku.stock -= int(count)
                    # sku.sales += int(count)
                    # sku.save()

                    # todo: 更新商品的库存和销量，乐观锁方式
                    orgin_stock = sku.stock

                    new_stock = orgin_stock - int(count)
                    new_sales = sku.sales + int(count)
                    # print('user:%d times:%d stock:%d orgin_stock:%d' % (user.id, i, sku.stock, orgin_stock))
                    # import time
                    # time.sleep(10)

                    # 更新sku并返回受影响的行数，如果返回为0不代表没有库存，返回重复查询3次
...
class OrderPayView(View):
    def post(self, request):
        ...
        # 初始化
        private_key_file = settings.BASE_DIR + r'\apps\order\app_private_key.pem'
        public_key_file = settings.BASE_DIR + r'\apps\order\alipay_public_key.pem'
        app_private_key_string = open(private_key_file).read()
        alipay_public_key_string = open(public_key_file).read()

        alipay = AliPay(
            appid="2016102300744128",  # 应用id
            app_notify_url=None,  # 默认回调url
            app_private_key_string=app_private_key_string,
            alipay_public_key_string=alipay_public_key_string,
            # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            sign_type="RSA2",  # RSA 或者 RSA2
            debug=True  # 默认False,沙箱使用True
        )
        # 调用支付接口
...
class CheckPayView(View):
    ...
    def post(self, request):
        ...
        # 调用支付宝的交易查询接口
        while True:
            response = alipay.api_alipay_trade_query(order_id)  # 支付宝返回的是字典类型
...
class CommentView(View):
    ...
    def post(self, request, order_id):
        ...
        for i in range(1, total_count + 1):
            ...
            order_goods.save()  # 保存到数据库

            order.order_status = 5  # 订单状态：已完成
            order.save()
